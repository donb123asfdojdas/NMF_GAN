# -*- coding: utf-8 -*-
"""
Inference for DNN-NSR 4x8 matrix completion

- 读取 ../../HCP-IMSC-main/GAN/ 目录中的 L*, M*, H* CSV
- 使用训练好的 dnn_nsr_4x8.pth 做矩阵补全
- 输出补全后的 CSV 到 ../HCP-IMSC-main/NMFdata&totalindex/output/
"""
...
# --------------------- 主推理流程 ---------------------
def main():
    device = torch.device("cpu")  # 有 GPU 可改为 torch.device("cuda")

    # 1. 加载训练好的模型
    model = DNNNSR().to(device)
    state = torch.load("dnn_nsr_4x8.pth", map_location=device)
    model.load_state_dict(state)
    model.eval()
    print("模型已加载: dnn_nsr_4x8.pth")

    # 2. 路径配置
    gan_folder = "../../HCP-IMSC-main/GAN/"
    output_folder = "../../HCP-IMSC-main/NMFdata&totalindex/output_FCNN/"
    ...
    print("All completed CSV files have been saved to:", output_folder)
