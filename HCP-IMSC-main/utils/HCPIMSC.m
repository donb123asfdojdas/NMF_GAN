function [X, history,min_error] = HCPIMSC(Xo, Po, Xu, Pu, lambda1, lambda2,unfindIndex,c,fileIndex,num)
% Inputs:
...
for iter = 1 : 2000
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %if iter == 1
        Weight = constructW_PKN(R,num-2);%K最临近
        Diag_tmp = diag(sum(Weight));
        L = Diag_tmp - Weight;
    %else
        %param.num_view = 10;%%%%%%%%%%%%%%%%%与实际视角要匹配
       % HG =     _nn_hypergraph(R', param);
       % L = HG.L;
   % end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % update Xu
...
%转换为网格对应的距离
%X=standardizeDistances(X,unfindIndex, sortedDistances);
%求取误差
min_error=calculateTotalRMSE(X,unfindIndex,fileIndex);
end